...
export class LibMongoModule {
	static forRootAsync(): DynamicModule {
		return {
			...
			imports: [
				...
				...Object.values(ENUM_CONNECTION_NAME).map((connectionName) =>
	...
	private static MongooseForRootAsyncFactory(
	...
	static forFeatureAsync({
		...
		return {
			module: LibMongoModule,
			imports: Object.values(ENUM_CONNECTION_NAME).map((connectionName) =>
	...
	static forFeature({
		name,
		schema,
		connectionName = ENUM_CONNECTION_NAME.PRIMARY,
	}: LibMongoModuleForFeatureOptions): DynamicModule {
		return {
			module: LibMongoModule,
			imports: [
