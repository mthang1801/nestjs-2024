...
export class LibRedisService {
	...
	constructor(
		@Inject(CommonConfig.NEST_TOKEN.REDIS_CONNECTION_OPTIONS)
		protected readonly redisConnectionOptions: RedisConnectionOptions,
		private readonly redisUtil: LibRedisUtil,
		private readonly configService: ConfigService,
	) {
		this.cacheTTL = Number(configService.get('REDIS_TTL')) ?? 0;
	}

	setTTL(ttl: number) {
		this.cacheTTL = Math.max(ttl, 0);
	}
	...
	public async set(
		...
		try {
			...
			return this.redisClient.SET(key, setValue, {
				EX: this.cacheTTL,
				...options,
			} as Partial<SetOptions>);
		} catch (error) {
	...
	public async getSet(
		...
		this.logger.log('************ GETSET *************');
		try {
			const setValue = this.redisUtil.setValue(value);
			const response = await this.redisClient.GETSET(key, setValue);
			await this.expire(key, options?.EX ?? this.cacheTTL);
			return response;
		} catch (error) {
			console.log(error.stack);
			throw new BadRequestException(error);
	...
	public async expire(
		key: string,
		duration: number,
		mode: RedisNS.ExpireTime = 'SECOND',
	) {
		return mode === 'SECOND'
	 ...
	 * @param {string} match
	 * @param {number} count
	 * @returns {Promise<RedisNS.ScanResponse>}
	 */
	public async scan(
		match = '*',
		cursor = 0,
		count = 20,
	): Promise<RedisNS.ScanResponse> {
		try {
	 ...
	 * @param {string} key
	 * @param {string[]} fields
	 * @returns {Promise<RedisNS.HScanResponse>}
	 */
	public async hScan(
		key: string,
		match = '*',
		cursor = 0,
		count = 20,
	): Promise<RedisNS.HScanResponse> {
		try {
			const hScanResponse: RedisNS.HScanReply = await this.redisClient.HSCAN(
				key,
				cursor,
				{
					MATCH: match,
	...
	async pop(key: string): Promise<any> {
		...
		return popElement && utils.parseData(popElement);
	}
	...
	async shift(key: string): Promise<any> {
		...
		return shiftElement && utils.parseData(shiftElement);
	}
	...
	async lSet(key: string, index: number, element: any): Promise<boolean> {
		const formatElement = JSON.stringify(element);
  ...
  * count = 0: Remove all elements equal to element.
  ...
  Note that non-existing keys are treated like empty lists, so when key does not exist, the command will always return 0.
	...
	async lRem(key: string, element: any, count = 0) {
		return this.redisClient.LREM(key, count, JSON.stringify(element));
	}
